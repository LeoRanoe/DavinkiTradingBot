...
/**
 * Authenticated bridge for Supabase Cron - RESEARCH REPORT.
 *
 ...
 *
 * NOT scheduled. Owner-driven: invoke to assemble the hard-test report from
 * everything already persisted. The upstream job is entirely read-only - it
 * computes a recommendation but has no power to act on one, and can never
 * open, close or size a position, nor change a strategy's status.
 */
/**
 ...
 * 70% of job invocations were lost to a transient gateway timeout on one of
 * them, with the job never running at all.
 *
 ...
 * costs nothing and cannot double-execute anything. The upstream research call
 * is left single-shot for the same reason it is in the research proxy, even
 * though report assembly itself is read-only: one report per invocation.
 */
...
Deno.serve(async (request: Request) => {
  try {
    ...
    const upstream = await fetch("https://davinki-trading-bot.vercel.app/api/jobs/research-report?format=text", {
      method: "POST",
      headers: {
        authorization: `Bearer ${tokenBody.access_token}`,
        "content-type": "application/json",
        "user-agent": "davinki-supabase-report-proxy/1.0",
      },
      body: "{}",
    });
    ...
    return Response.json(
      { error: "Research report proxy failed", detail: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
});
