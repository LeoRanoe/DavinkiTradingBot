...
/**
 * Authenticated bridge for Supabase Cron - RESEARCH BACKFILL.
 *
 ...
 *
 * NOT scheduled. This is an owner-driven, resumable job: invoke it repeatedly
 * until every target reports COMPLETE. It only ever extends history
 * backwards and can never touch the live scanner watermark.
 */
/**
 ...
 * costs nothing and cannot double-execute anything. The upstream backfill call
 * is deliberately NOT retried here - it is the side-effecting one, and
 * leaving it single-shot keeps two backfill runs from ever overlapping.
 */
...
Deno.serve(async (request: Request) => {
  try {
    ...
    // Forwarded so the caller can tune targetDays / maxPages per invocation.
    ...
    const upstream = await fetch("https://davinki-trading-bot.vercel.app/api/jobs/research-backfill", {
      method: "POST",
      headers: {
        authorization: `Bearer ${tokenBody.access_token}`,
        "content-type": "application/json",
        "user-agent": "davinki-supabase-backfill-proxy/1.0",
      },
      body: forwardedBody,
    });
    ...
    return Response.json(
      { error: "Backfill proxy failed", detail: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
});
