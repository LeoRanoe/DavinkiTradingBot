...
/**
 * Authenticated bridge for Supabase Cron - RESEARCH RUN.
 *
 ...
 *
 * NOT scheduled. Owner-driven analysis: invoke once per research stage. The
 * upstream job is read-only with respect to trading and can never open,
 * close or size a position.
 */
/**
 ...
 * leaving it single-shot keeps two research runs from ever overlapping.
 */
...
Deno.serve(async (request: Request) => {
  try {
    ...
    // Forwarded so the caller can tune the research stage per invocation.
    ...
    const upstream = await fetch("https://davinki-trading-bot.vercel.app/api/jobs/research-run", {
      method: "POST",
      headers: {
        authorization: `Bearer ${tokenBody.access_token}`,
        "content-type": "application/json",
        "user-agent": "davinki-supabase-research-proxy/1.0",
      },
      body: forwardedBody,
    });
    ...
    return Response.json(
      { error: "Research proxy failed", detail: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
});
