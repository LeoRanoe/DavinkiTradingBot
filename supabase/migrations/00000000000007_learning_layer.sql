...
alter table signals add column if not exists owner_decision_at timestamptz;
alter table signals add column if not exists owner_rejection_reason text;
...
alter table trades add column if not exists equity_after numeric;
...
create or replace function public.queue_rejected_counterfactual() returns trigger language plpgsql security definer set search_path = public as $$
begin
  if old.approval_status = 'PENDING' and new.approval_status = 'REJECTED' then
    insert into counterfactual_outcomes(signal_id, source, rejection_reason, plan_snapshot)
    values (new.id, case when new.owner_rejection_reason is null then 'OWNER_REJECTED' else 'RISK_BLOCKED' end, new.owner_rejection_reason,
...
create trigger signals_queue_rejected_counterfactual after update of approval_status on signals for each row execute function public.queue_rejected_counterfactual();
